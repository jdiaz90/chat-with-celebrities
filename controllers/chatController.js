 ...
 * @description Controlador de chat: render de la vista y streaming de respuestas del modelo.
 */
 ...
 * - Inyecta el usuario autenticado desde res.locals para no sobrescribir el valor del middleware.
 *
...
export async function renderChatPage(req, res) {
  ...
  res.render('chat', {
    title: `Chat con ${celebrity.name || 'una persona famosa'}`,
    celebrities: Object.values(celebrityMap),
    celebrityMap,
    selected: celebrity,
    selectedId: id,
    user: res.locals.user // ✅ no pisar el user inyectado por el middleware global
  });
}
 ...
 * 2) Envía cabeceras para streaming de texto al producir el primer chunk (no antes).
...
export async function streamChatResponse(req, res) {
  ...
  try {
    ...
    // Nota: diferimos el envío de cabeceras hasta que tengamos el primer chunk válido
    const responseStream = await generateResponse(resolvedModel, promptText);
    const stream = Readable.from(responseStream);

    let started = false; // control de envío de cabeceras/status
    let buffer = '';
    ...
    if (started) {
      ...
      // No se produjo ningún chunk (vacío pero sin error): responder 200 vacío coherente con text/plain
      res.setHeader('Content-Type', 'text/plain; charset=utf-8');
      res.status(200).send('');
    }
